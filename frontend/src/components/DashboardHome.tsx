...
const DashboardContent: React.FC = () => {
  ...
  // Generate layout for charts + data table
  const chartLayoutConfig = useMemo(() => {
    if (!dashboardConfig?.charts) return []
    
    const layouts: LayoutItem[] = dashboardConfig.charts.map((chart, idx) => {
      const colsPerRow = 2
    ...
    // Add DataTable as full-width widget at bottom
    layouts.push({
      i: 'data-table',
      x: 0,
      y: Math.ceil(dashboardConfig.charts.length / 2) * 4,
      w: 12,
      h: 3,
      minW: 6,
      minH: 2,
  ...
  return (
    <div className="vibha-dashboard-container" style={{ maxWidth: '1400px', margin: '0 auto', padding: '2rem' }}>
      ...
      {/* ZONE 3: Charts + Data Table (All Resizable) */}
      <div className="vibha-zone-charts-wrapper">
        <h3 style={{ marginBottom: '1.5rem' }}>Charts, Data & Visualizations</h3>
        <p style={{ fontSize: '0.9rem', color: '#666', marginBottom: '1rem' }}>
          💡 Drag headers to move • Drag corners/edges to resize (PowerBI-style) • Click elements to filter
        </p>
        ...
        >
          ...
          {/* Data Table as Resizable Widget */}
          <Card
            key="data-table"
            title="Raw Data"
            subtitle="Full-width, resizable"
          >
